...
1. **样本内外分开比，主看收益风险比。** 同一套指标先切样本内/样本外（`oos_start` 之后才是真证据）。样本外显著失效是过拟合的强信号，但要结合有效样本量判断——样本外段太短不足以定论，别直接否。比指标时先看夏普、卡玛这类收益风险比，不看绝对收益。
...
3. **年化收益、最大回撤只作参考，不做绝对评判。** 这两类指标与杠杆使用强相关——同一策略加杠杆，年化和回撤同比放大。跨策略比较先对齐杠杆口径；最终拍板的尺子始终是收益风险比。
