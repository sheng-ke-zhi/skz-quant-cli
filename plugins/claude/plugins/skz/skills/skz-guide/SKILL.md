...
**CLI 是无状态的，但账号的读面就是会话状态。** 因子挖掘/探索动辄几十分钟，必然跨会话——人关了终端再回来，你是空白的，但**平台不是**。所以每次开场（尤其用户说"接着上次"），先读一遍账号，重建"他走到第几步"：

```bash
skz whoami                                   # key 活性 + 身份
skz mine runs --status active                # 此刻有因子挖掘在跑吗？→ 有就去 poll 它，别重复触发
skz explore runs --status active             # 此刻有探索在跑吗？
skz factor-routes list                       # 已有哪些研究方向（研究面 = 真源）→ [{code,name,...}]
skz mining runs                              # 有哪些挖掘成果（成果柜；策略面的 mine runs 可能是空的）
skz experiment list                          # 探索出过哪些实验（待评审的候选）
skz strategy list                            # 实盘库里已经有什么（含 暂停 态）
```

读完你就知道该接哪一步：**有在跑的 → 轮询它**；**有挖完没看的 → 去 `skz-factor` 看货**；**有实验没评审的 → 去 `skz-candidate`**；**全空 → 从①聊想法开始**。
...
> **所以重建位置以研究面为准**：方向看 `factor-routes list`（它的 `code` 就是 `mine/explore --route` 要的值）、探索成果看 `experiment list`、挖掘成果看 `mining runs`。
...
③ 和 ④/⑤ 是**两支**：**挖因子只要 route；策略探索要 problem + route**。problem 不是因子挖掘的前置。

### ① 聊清想法（纯对话 · 不碰 CLI）

这一步的产出**不是**一条 route，是**七个字段的原料**。原料不齐就去建 route，建出来的是一条没人能复核的方向——后端不校验这些字段的成色，糊弄照样 exit 0 收下，几十分钟的挖掘费照样扣。
...
**逃生门**：人说「别问了，直接建」→ 别争，看还缺哪个字段，只补问那一个最关键的（多半是 Q1），然后照做。这个逃生门只省第①步的追问：调用 `skz route create` 前仍须按②完整展示待建路线并取得明确许可，不能跳过。**跳步**：他第一句话已经答过的别再问一遍——不太懂的人被追问三轮会走，专业的人被问已知的东西会烦。

### ② 建研究方向（写 · 不花钱 · ⚠️ 必须先问人）

**⚠️ HITL：调用 `skz route create` 前，先按下方「给人看」的形状完整展示本次待建的每条路线（名字、看什么、钱从哪来、为什么还在、哪类机制、什么时候失效、标签七个字段都要有），取得明确许可后才调用。内容有修改后，必须重新展示并重新确认。即使用户说「直接建」或「别确认了」也不得跳过展示与许可；「直接建」只能省第①步的追问。**
...
**建之前先跟已有方向对一遍，别建重复的**——比的是**机制**不是名字，两条名字迥异的方向赚的可能是同一批对手盘的钱：
...
> **调用 `skz route create` 前逐字自检**：行为金融/微观结构术语是描述、不是枚举值，必须归并到上表 10 值之一。真实失败案例：❌ `投资者反应不足`（后端 INPUT_VALIDATION 拒绝）→ ✅ `行为偏差`。
...
三句都有用途：**第一句**给人一个能反驳的靶子——他可能答「等等，那个板块的量能一直是游资对倒，你这条对手盘不成立」，这一次「等等」就省下一次扣费，而「要花钱吗」这种问法永远换不到它。**第二句**是你自己的止损线，跑完照它验（数据在 `mining runs` 的 `retain_rate`/`retained` 与 `mining factors` 的 `mean_sharpe`，都在 `skz-factor` 里；**对人说的时候按前言〈跟人说话：字段名不出口〉那张表翻译，别把键名念出来**），免得挖完拿着一堆数字说不清算成还是没成。**第三句**让他知道等多久、以及等的是钱。

> **⚠️ 止损线写成「跟谁比」，别写成一个绝对数。** 因子挖掘的保留率天生就低——**3000 个候选留下 121 个（4%）是正常信噪比，不是失败**。拍一个「低于 x% 就算失败」的阈值，会让你在一次完全正常的挖掘之后宣布路线不成立，而钱已经花了。
> `mining runs` 一次就返回该账号所有 run 的保留率，**参照系是现成的**：跟自己其他 run 比、尤其跟同一条方向上次挖的比。绝对水平交给参照系，你只判**方向**（比同账号明显差、中心趋势为负）。这条同样适用于下面探索的通过率。

提炼不出核心假设 = 第①步没走完，回去补 Q1，别拿因子挖掘代替思考——挖掘不会告诉你对手盘是谁，它只会告诉你这批表达式在训练集上的分布。

**触发前后各有一道免费检查，见下面〈触发这两步的通用规矩〉**——因子挖掘和探索同一套，那节写一遍。
...
挖完（`ok:true`）看产出 → **`skz-factor`**（`mining overview <fcRunId>` / `mining factors <fcRunId>`）。
...
**⚠️ HITL：调用 `skz problem create` 前，先完整展示本次待建问题的标的集合、市场类型、频率、训练/验证时间分段及其余内容，取得明确许可后才调用。内容有修改后，必须重新展示并重新确认。即使用户说「直接建」或「别确认了」也不得跳过展示与许可。**
...
止损信号在 `skz-candidate` 那册（`experiment list` 的 `pass_rate`、`experiment strategies` 的 `verdict.yearly_metrics`）。**通过率同样低得离谱才是常态**——一次探索几十个回测里过一两个是正常量级，所以还是跟本账号其他实验比，别拍绝对阈值。**探索比因子挖掘更值得先问一句「这个问题配这条方向合不合」**——问题选错（品种、频率跟因子看的东西不匹配）会让一批本来不错的因子全军覆没，而这种失败长得跟「因子不行」一模一样。

探完评审候选 → **`skz-candidate`**（`experiment strategies <id>` → `review-matrix` → 保存入库）。

## 触发这两步的通用规矩（因子挖掘 / 探索同一套）
...
- **每完成一步，主动提下一步**：给完方向 → 选一条挖因子；挖完 → 可再挖、或定义 problem 做探索；探完 → 去 `skz-candidate` 评审、保存入库；入库后再交给 `skz-strategy` 暂停观察，真上场是另一个决定。
