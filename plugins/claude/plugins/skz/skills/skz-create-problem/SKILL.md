...
市场类型（`dataset`：股票 / 期货 / ETF）决定后端自动设置的策略命名前缀：期货 `FTS`、股票 `STS`、ETF `ETS`。前缀中间段为交易周期（如 `D` = 日线、`60` = 60 分钟、`15` = 15 分钟）——这只是命名示例，不是 `freq` 合法值的清单，别拿前缀反推合法频率；`freq` 的合法项以 `skz problem meta` 返回的 `freq_options` 为准（见下）。

## 创建命令（写 · 不花钱 · ⚠️ 必须先问人）

**⚠️ HITL：调用 `skz problem create` 前，先用人话完整展示待建问题的标的集合、市场类型、频率、训练/验证时间分段及其余内容，取得明确许可后才调用。内容有修改后，必须重新展示并重新确认；既有的创建前自检三问不能替代用户许可。即使用户说「直接建」或「别确认了」也不得跳过展示与许可。**
...
4. 品种数量上限（平台硬上限）：日线不超过 100 个，60 分钟不超过 50 个，15 分钟不超过 30 个；120/240 分钟档的上限未单列，以后端校验为准。
...
时序问题 `symbols` 一般 ≤10 个、应为高相关品种——这是**经验建议**（高相关才构成同类风险暴露），不是平台限制；平台硬上限见上面第 4 条。截面问题**最少 10 个**（因子值需横截面可比）。注意截面多空暂不在平台支持的研究流程内（见文末「适用范围」）；`problem_type` 的可用项以 `skz problem meta` 实际返回为准——meta 仍可能列出 `CrossSectionalProblem`，枚举存在不等于流程已支持。
...
本册面向平台当前支持的**时序量价 CTA / 择时研究流程**；股票截面选股与截面多空策略暂不在该流程内，研究问题应与实际可支持的策略类型保持一致。
