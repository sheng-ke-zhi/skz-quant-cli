---
title: 因子挖掘任务列表
endpoint: GET /strategy/miner/runs
...
```json
{
  page, size, total,
  items: [{
    fcRunId, routeCode, status, statusText, done, ok,
...
本接口来自任务调度侧，返回的是任务本身的执行状态（排队、运行中、失败原因、结果路径），支持按状态过滤与分页，适合做任务列表页与进度轮询。

若要看每次挖掘产出了多少因子（候选数、保留数、保留率、耗时），请用投研接口的因子挖掘产出记录。
