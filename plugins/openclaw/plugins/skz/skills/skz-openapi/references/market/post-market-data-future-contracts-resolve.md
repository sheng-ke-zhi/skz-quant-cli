...
该接口的主要用途是**实盘合约换算**：999 期货主力、888 期货指数这类平台规范标的并非交易所真实合约，无法直接下单；本接口将其解析为最新时点下可直接通过 CTP 实盘交易的交易所标准合约——999 主力映射为单一当前主力合约（权重 1），888 指数映射为一篮子成分合约及各自的 `index_weight` 权重。
