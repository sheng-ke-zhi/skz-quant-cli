---
title: 挖掘结果概览
endpoint: GET /research/mining/{run_id}/overview
...
完整地址：`GET https://api.shengkezhi.com/open/v1/research/mining/{run_id}/overview`
...
```json
{
  ...
  "data": {
    "elimination_breakdown": [
      { "count": 53, "kind": "retained", "reason": "终选保留", "stage": "filter" },
      { "count": 2178, "kind": "eliminated", "reason": "高相关冗余", "stage": "duplicate" },
      { "count": 0, "kind": "eliminated", "reason": "体检淘汰", "stage": "detect" }
    ],
    "funnel": [
      { "eliminated": 0, "remaining": 2900, "stage": "生成", "step": "build" },
      { "eliminated": 2178, "remaining": 722, "stage": "去重初筛", "step": "duplicate" },
      { "eliminated": 0, "remaining": 722, "stage": "因子体检", "step": "detect" }
    ],
    "kpi": {
      "eliminated": 2847,
      "evaluate_methods": ["MA001", "TS001"],
      "problem_count": 16,
    ...
    "problem_groups": [
      { "count": 3, "label": "ETF时序", "prefix": "ETS" },
      { "count": 10, "label": "期货时序", "prefix": "FTS" },
      { "count": 3, "label": "股票时序", "prefix": "STS" }
    ],
    "route": {
      "code": "fde0cb170b25",
      ...
      "failure_scenarios": ["消息刺激造成短暂假突破", "宽幅震荡中均线频繁转向"],
