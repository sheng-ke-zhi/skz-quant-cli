---
title: 删除策略研究执行
endpoint: DELETE /research/experiments/{id}
...
实际调用地址：`https://api.shengkezhi.com/open/v1/research/experiments/{id}`。需在请求头携带 `Authorization: Bearer sk_xxx`。
...
| force | query | boolean | 否 | - | 越过「目录最近仍有写入」的软护栏。该护栏是启发式判断（后端不触发探索任务，无从确知是否真有任务在跑），必须留逃生舱，否则会造出「明知是垃圾却删不掉」的死角。 |
